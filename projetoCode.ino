...
void reconnect() {
  ...
  while (!client->connected()) {
    ...
    if (client->connect(clientId.c_str(), "lugaba1", "@Mackenzie20")) {
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    if (cmMsec < 10) {
      ...
      lastMsg = now;
      ++value;
      snprintf (msg, MSG_BUFFER_SIZE, "Opa! A tampa foi aberta! #%ld", value);
      Serial.print("Publish message: ");
      Serial.println(msg);
      client->publish("MACK32016816/SENSOR1", msg);  
      delay(3000);
    } else {
      servo.write(180);    
      delay(1000);
